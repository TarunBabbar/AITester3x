...
def build_llm(settings: Settings, max_tokens: int | None = None) -> LLM:
    ...
    if settings.llm_base_url:
        ...
        # CrewAI strips the "provider/" prefix from model ids before sending
        # (deepseek/deepseek-v4-flash -> deepseek-v4-flash), which the Command
        # Code Provider API rejects. custom_openai sends the model id verbatim
        # to the configured base_url, so the full prefixed id survives.
