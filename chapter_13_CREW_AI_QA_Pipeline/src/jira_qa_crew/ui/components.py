...
def render_config_panel(settings: Settings) -> None:
    ...
    # MCP is only in play when the mode is auto (MCP -> REST fallback) or mcp.
    # In rest-only mode there is no MCP path, so the row would only ever show
    # a misleading red "not configured".
    blocks = [("LLM", "llm"), ("Jira REST", "jira_rest")]
    if settings.jira_integration_mode in (IntegrationMode.AUTO, IntegrationMode.MCP):
        blocks.append(("Jira MCP", "jira_mcp"))
